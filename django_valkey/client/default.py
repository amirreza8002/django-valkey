...
class DefaultClient:
    def __init__(self, server, params: Dict[str, Any], backend: BaseCache) -> None:
        ...
        self._clients: List[Optional[Valkey]] = [None] * len(self._server)
    ...
    def connect(self, index: int = 0) -> Valkey:
        """
    ...
    def disconnect(self, index: int = 0, client: Optional[Valkey] = None) -> None:
        """
    ...
    def persist(
        self, key: KeyT, version: Optional[int] = None, client: Optional[Valkey] = None
    ) -> bool:
        if client is None:
            client = self.get_client(write=True)
    ...
    def clear(self, client: Optional[Valkey] = None) -> None:
        """
